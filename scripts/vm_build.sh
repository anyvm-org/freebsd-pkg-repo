...
if [ -n "${BLACKLIST:-}" ] && [ -f "${BLACKLIST}" ]; then
    ...
    # A blacklisted port that has since been BUILT elsewhere and published
    # must lose its IGNORE line, or its dependents stay unbuildable
    # forever: the seed would hand poudriere a perfectly good package while
    # the ports framework still calls the port ignored, and every dependent
    # is skipped with "Dependent port ... ignored". That is exactly what
    # happened the round after lang/gcc14 was imported (run 34339210051):
    # 18 jobs, 1,323 ports queued, 307 skipped per job, zero built.
    EFFECTIVE="${OUTDIR}/blacklist.effective"
    if [ -n "${LEDGER:-}" ] && [ -f "${LEDGER}" ]; then
        python3 - "${BLACKLIST}" "${LEDGER}" > "${EFFECTIVE}" <<'PY'
import json
import sys
...
for origin in origins:
    entry = ports.get(origin) or ports.get(origin + "@default")
    if entry and entry.get("state") == "built" and entry.get("pkgfile"):
        published.append(origin)
    else:
        print(origin)
if published:
    print("blacklist: %d entries are published now, not ignoring them: %s"
          % (len(published), " ".join(published)), file=sys.stderr)
PY
    else
        grep -v '^[[:space:]]*#' "${BLACKLIST}" | sed -e 's/[[:space:]]*#.*//' -e '/^[[:space:]]*$/d' > "${EFFECTIVE}"
    fi
    {
        echo ""
...
# Ports the ledger has parked (oversize or failed on this machine),
# written by the plan job. Same IGNORE mechanism: poudriere then skips
# their dependents at once instead of rebuilding the blocker as a
# dependency in every job (round 15: grpc, 4h53m to its timeout, in
# four jobs at the same time; 114 packages from 18 jobs).
if [ -n "${BLOCKED:-}" ] && [ -f "${BLOCKED}" ]; then
    mkdir -p /usr/local/etc/poudriere.d
    {
        echo ""
        echo "# generated by vm_build.sh from the ledger's oversize and failed ports"
        grep -v '^[[:space:]]*#' "${BLOCKED}" | sed -e '/^[[:space:]]*$/d' |
        while read -r origin; do
            echo ".if \${.CURDIR:M*/${origin}}"
            echo "IGNORE=	parked by the ledger: oversize or failed on the CI runner"
            echo ".endif"
        done
    } >> /usr/local/etc/poudriere.d/make.conf
    echo "parked: $(grep -vc '^[[:space:]]*#' "${BLOCKED}") oversize/failed origins set IGNORE too"
fi
